...
/**
 * Deletes future events with a given title.
 */
function deleteReoccurringEvent() {
  var calendarName = "BHFH Resident Calendar"
  var calendar = CalendarApp.getCalendarsByName(calendarName).pop()
  var title = "YAF Quaker Worship"

  // Define how many days into the future the script should look
  var numberOfDaysToLook = 180

  // Set up the copying date span
  var today = new Date()
  var startDate = today
  var endDate = new Date()
  endDate.setDate(startDate.getDate() + numberOfDaysToLook)
  ...
  while (events.length > 0) {
    ...
    // Delete events that have a matching title
